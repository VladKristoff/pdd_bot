...
from bot.utils.test_system import TestStates, show_question, get_correct_answer_id
...
@exam_router.callback_query(F.data == "start_exam")
async def start_exam(callback: CallbackQuery,state: FSMContext):
    global ticket_number

    ticket_number = f"Билет {random.randint(1, 40)}"  # Получение случайного билета

    test_manager = TestManager(question_requests)

    try:
        question = await test_manager.start_ticket(ticket_number)
    except Exception as e:
        print(f"Ошибка в загрузке билета {ticket_number}: {e}")
...
@exam_router.message(TestStates.waiting_for_answer, F.text.in_(["1","2","3","4"]))
...
@exam_router.message(TestStates.showing_explanation, F.text == "Следующий")
